---
name: defect-rate-conventions
description: >-
  defect-rate-ia architecture (Next.js): dumb/smart, hooks, axios services,
  barrels, Tailwind primitives, cn(), no code comments. Canonical spec: CLAUDE.md
---

# defect-rate-conventions (Cursor)

**Source of truth:** [CLAUDE.md](../../../CLAUDE.md)

1. [CLAUDE.md](../../../CLAUDE.md) — full specification
2. [docs/architecture.md](../../../docs/architecture.md) — overview
3. [docs/architecture-reference.md](../../../docs/architecture-reference.md)
4. [docs/architecture-examples.md](../../../docs/architecture-examples.md)
